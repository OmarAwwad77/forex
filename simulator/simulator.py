...
from simulator.trade import Trade, TradeStatus, TradeOutcome
...
class Simulator:
    ...
    def run(self) -> Tuple[List, List]:
        ...
        print(f'Open Trades len: {len(open_trades)}\nClosed Trades len: {closed_trades_len}')
        if closed_trades:
            print(
            f'{bad_trades_len} ({bad_trades_len / closed_trades_len * 100:.2f}%) wrong trades taken out of: {closed_trades_len} trades in total')
            # if self.pickle_file:
            #     self._save_simulation_results(closed_trades)
        return open_trades, closed_trades


    def save_simulation_results(self, trades: List, pickle_file: pathlib.Path):
        pickle_file.parent.mkdir(parents=True, exist_ok=True)
        current_date_time = datetime.now().strftime("%d_%b_%Y(%I:%M%p)")
        original_file_name = pickle_file.stem
        file_extension = pickle_file.suffix
        modified_file_path = pickle_file.parent / f'{original_file_name}_{current_date_time}{file_extension}'
        print(f'Creating {modified_file_path}...')
